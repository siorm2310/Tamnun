...
def Classifier(items):
    ...
    for i in range(len(items["items"])):
        if items["father"][i] == items["items"][i]:
            ...
            priority.append(i)
    classified = {"priority": priority, "childless": childless,
                  "father": father, "truth": truth, "delta": delta}
    return classified
...
def Delta_Derivative(derivative, i, items):
    ...
    for j in range(l):
        if items["items"][i] in (derivative[j])["items"]:
            ...
            m = float(items["weight_delta"][i])*float(items["cg"][i])+float(items["weight"][i]) * \
                float(items["cg_d"][i])+float(items["weight_delta"][i])*float(items["cg_d"][i])
            dict3 = {
                "items": (derivative[j])["items"],
                "weight": (derivative[j])["weight"]+float(items["weight_delta"][i]),
...
def Derivative_Generator_P1(items, classified):
    '''
    ...
    for j in range(len(itemlist)):
        ...
        for i in classified["priority"]:
            if items["father"][i] == item:
                ...
                string = string.strip(',')
                weight = float(itemlist[j]["weight"]) + \
...
def Physical_Derivative(items):
    ...
    Derivatives = Derivative_Generator_P1(items, classes)
...
def Derivative_Generator(items):
    '''
...
# creating a dictonty for the planes
    plane = {"Tail_num": (items["aircrafts"][0])["Tail_num"], "Weight": (
        items["aircrafts"][1])["Weight"], "CG": (items["aircrafts"][2])["CG"]}
    ...
    for i in range(len(plane["Tail_num"])):
        ...
        for j in range(len(Derivatives)):
            ...
            cg = (float(plane["Weight"][i])*float(plane["CG"][i])+float((Derivatives[j])
                                                                        ["moment"]))/(float((Derivatives[j])["weight"])+float(plane["Weight"][i]))
