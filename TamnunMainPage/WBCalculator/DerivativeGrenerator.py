...
def Items(item,items,classified):
    ...
    for i in classified["priority"]:
        if items["father"][i]==lastitem:
            ...
            weight=float(item["weight"])+(float(items["weight"][i])+float(items["weight_delta"][i]))
            m=(float(items["weight"][i])+float(items["weight_delta"][i]))*(float(items["cg"][i])+float(items["cg_d"][i]))
...
# Main
datajson=open("C:/Users/Gilad Timar/Documents/עבודה/scripts/dummyClientRequest2.json", 'r')
